...
const server = app.listen(PORT, () => console.log(`CRM Tasatop Web v1.444 (Ajustes Comité B2B: (1) Cabecera sticky ahora baja 60px al scrollear (queda completa debajo del topbar del CRM, ya no a medias). (2) La proyección a cierre del equipo SOLO suma el enCamino (Reunión+Finanzas) de los funcionarios CON meta individual (Shirley y Bony); los leads de Dante u otros sin meta quedan fuera de la estimación, con el subtítulo indicando de quiénes es el potencial. (3) Gestión diaria cuadrada: ejes X sin rotación con máx 7 ticks, Y con máx 5, etiquetas de datos más pequeñas con clamp. (4) Gestión por hora SIN barra de desplazamiento: celdas fluidas minmax(0,1fr), etiquetas de hora alternas cuando hay más de 12 columnas, tooltip conserva el detalle. (5) KPI cards más chatas (64px) con el icono a la DERECHA. (6) Insights IA acotados a su recuadro (max 300px con degradado) y botón Ver análisis completo que abre modal flotante. (7) EMBUDO COMERCIAL con dos columnas nuevas de tiempo: ⏳ EN ETAPA (días promedio de los vivos estancados en cada etapa, semáforo verde<4d/ámbar/rojo≥7d) y 🚀 LLEGADA (días promedio desde el ingreso hasta ALCANZAR esa etapa, reconstruido de la auditoría histórica de movimientos de kanban con fallback a fechaEtapa sellada) — mide qué tan rápido se mueven los leads hacia etapas avanzadas. Front: Ctrl+F5) corriendo en puerto ${PORT}`));
